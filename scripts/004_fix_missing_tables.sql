...
DROP POLICY IF EXISTS "questions_select_own" ON public.interview_questions;
DROP POLICY IF EXISTS "questions_insert_own" ON public.interview_questions;
DROP POLICY IF EXISTS "questions_update_own" ON public.interview_questions;
DROP POLICY IF EXISTS "questions_delete_own" ON public.interview_questions;
DROP POLICY IF EXISTS "responses_select_own" ON public.interview_responses;
DROP POLICY IF EXISTS "responses_insert_own" ON public.interview_responses;
DROP POLICY IF EXISTS "responses_update_own" ON public.interview_responses;
DROP POLICY IF EXISTS "responses_delete_own" ON public.interview_responses;
...
-- Create trigger for updated_at on interview_responses (only if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_responses_updated_at' 
    AND tgrelid = 'public.interview_responses'::regclass
  ) THEN
